        ...
        if self:
            for record in self:
                ...
                if len(fiscal_position_ids)>1 and pos_invoice_obj:
                    pos_order_obj = self.env['pos.order'].search([('invoice_id','!=',False),('session_id','=',record.id),('fiscal_position_id','in',fiscal_position_ids)], order='invoice_id asc')
                elif len(fiscal_position_ids)==1 and pos_invoice_obj:
                    pos_order_obj = self.env['pos.order'].search([('invoice_id','!=',False),('session_id','=',record.id),('fiscal_position_id','=',fiscal_position_ids)], order='invoice_id asc')
                else:
                    return invran
    ...
    def get_total_sales_invoice_gravado_no_contr(self):
        ...
        if self:
            for record in self:
                ...
                elif len(fiscal_position_ids)==1 pos_order_obj:
